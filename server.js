...
if (process.env.DISABLE_GUI && process.env.DISABLE_GUI !== 'false') {
  app.get('*', async (req, res) => {
    ...
    const htmlContent = placeholderContent.replace(
      '<!-- CONTENT -->', 
      'website-check API is up and running!<br />Access the endpoints at '
...
const printMessage = () => {
  console.log(
    ...
    `\x1b[0m\n`,
    `\x1b[1m\x1b[32m🚀 Website-Check is up and running at http://localhost:${port} \x1b[0m\n\n`,
    `\x1b[2m\x1b[36m🛟 For documentation and support, visit the GitHub repo: ` +
    `https://github.com/bibo318/website-check \n`,
    `💖 Found website-check useful? Consider sponsoring us on GitHub ` +
