...
const Results = (): JSX.Element => {
  ...
  return (
    <ResultsOuter>
      ...
      { address?.includes(window?.location?.hostname || 'website-check.xyz') && <SelfScanMsg />}
      <Loader show={loadingJobs.filter((job: LoadingJob) => job.state !== 'loading').length < 5} />
      <FilterButtons>{ showFilters ? <>
        <div className="one-half">
        <span className="group-label">Lọc theo</span>
        ...
        {(tags.length > 0 || searchTerm.length > 0) && <span onClick={clearFilters} className="clear">Xóa bộ lọc</span> }
        </div>
        <div className="one-half">
        <span className="group-label">Tìm kiếm</span>
        <input 
          type="text" 
        ...
        <span className="toggle-filters" onClick={() => setShowFilters(false)}>Hide</span>
          ...
          <div className="control-options">
            <span className="toggle-filters" onClick={() => setShowFilters(true)}>Hiển thị bộ lọc</span>
            <a href="#view-download-raw-data"><span className="toggle-filters">Export Data</span></a>
            <a href="/about"><span className="toggle-filters">Tìm hiểu về kết quả</span></a>
            <a href="/about#additional-resources"><span className="toggle-filters">Thêm công cụ</span></a>
            <a href="https://github.com/bibo318/website-check"><span className="toggle-filters">Xem GitHub</span></a>
          </div>
      ) }
      </FilterButtons>
      <ResultsContent>
